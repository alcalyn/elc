...
class TicTacToeAjaxController extends Controller
{
	...
	public function refreshAction(PhaxAction $phax_action)
    {
    	...
    	$winner = self::_winner($party->getGrid());
    ...
    public function tickAction(PhaxAction $phax_action)
    {
    	...
    	if ($grid[$index] !== '-') {
    		return $phax->error('case already checked by '.$grid[$index].' in grid '.$grid.' at position '.$line.' ; '.$col);
    	}
    ...
    private static function _winner($grid)
    {
    	...
    	if (self::_brochette($grid, 3, 4, 5)) return $grid[3];
    	if (self::_brochette($grid, 6, 7, 8)) return $grid[6];
    	
    	if (self::_brochette($grid, 0, 3, 6)) return $grid[0];
    	if (self::_brochette($grid, 1, 4, 7)) return $grid[1];
    	if (self::_brochette($grid, 2, 5, 8)) return $grid[2];
